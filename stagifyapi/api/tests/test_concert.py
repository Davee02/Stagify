...
class ConcertTest(TestCase):
    def setUp(self):
        artist_user = User.objects.create_user(
            username="artistuser", password="artistuser"
        )
    ...
    def test_create_concert_with_no_user_returns_401_unauthorized(self):
        ...
        response = self.client.post(
            "/api/concerts/", payload, content_type="application/json"
        )
    ...
    def test_update_concert_with_no_user_returns_401_unauthorized(self):
        ...
        response = self.client.put(
            "/api/concerts/1", payload, content_type="application/json"
        )
        response_object = json.loads(response.content)
    ...
    def test_get_suggestions_with_custom_amount_concerts_returns_correct_amount_of_concerts_and_200_ok(
        self,
    ):
        login = self.client.login(username="normaluser", password="normaluser")
    ...
    def test_get_suggestions_with_too_big_amount_concerts_returns_all_concerts_and_200_ok(
        self,
    ):
        login = self.client.login(username="normaluser", password="normaluser")
