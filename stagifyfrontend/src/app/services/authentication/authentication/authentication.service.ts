...
export class AuthenticationService {
  ...
  async login(model: LoginViewModel): Promise<HttpResponse<any>> {
    return await this.httpClient
      .post<any>(this.apiUrl + '/user/login', JSON.stringify(model), {
  ...
  async register(model: RegisterViewModel): Promise<HttpResponse<any>> {
    return this.httpClient
      .post<any>(this.apiUrl + '/user/register', JSON.stringify(model), {
        headers: new HttpHeaders().set('Content-Type', 'application/json'),
        observe: 'response',
      })
      .toPromise();
  ...
  async logout(): Promise<HttpResponse<any>> {
    ...
    return this.httpClient
      .post<any>(this.apiUrl + '/user/logout', { observe: 'response' })
      .toPromise();
  }
}
